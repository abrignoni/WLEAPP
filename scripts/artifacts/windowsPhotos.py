...
def get_windowsPhotos(files_found, report_folder, seeker, wrap_text):
    ...
    for file_found in files_found:
        ...
        if usageentries > 0:
            ...
            data_headers = ('Filename', 'Filesize', 'Width', 'Height', 'Latitude', 'Longitude', 'ApplicationName', 'CameraManufacturer', 'CameraModel', 'DateTaken', 'DateCreated', 'DateModified', 'DateIngested')
            for rows in all_rows:
                data_list.append((rows[0], rows[1], rows[2], rows[3], rows[4], rows[5], rows[6], rows[7], rows[8], rows[9], rows[10], rows[11], rows[12]))
